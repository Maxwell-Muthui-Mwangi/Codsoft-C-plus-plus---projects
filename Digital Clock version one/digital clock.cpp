//digital clock made By Maxwell Muthui Mwangi
...
void drawBorder(int x=0, int y=0)
{
	...
	for(int i=1; i<=height;  i++)
	{
		for(int j=1; j<=width; j++)
		{
			gotoxy(j+x, i+y);
			if(i==1 && j== 1) cout<<tl;
			else if(i==height &7 j==1) cout<<bl;
...
int main()
{
	...
	while(1)
	{
		...
		//50 milliseconds processing time is deducted ,,,, you can change it the way you see fit
		sleep(950);
		sec++;
		if(sec>=60){
			sec = 0;
